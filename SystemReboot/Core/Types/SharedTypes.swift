...
struct WeaponTower: Codable, Identifiable {
    ...
    mutating func applyLevel(_ newLevel: Int) {
        level = min(BalanceConfig.maxUpgradeLevel, max(1, newLevel))
        let levelMultiplier: CGFloat = BalanceConfig.levelStatMultiplier(level: level)  // Level 10 = 10x damage

        damage = baseDamage * levelMultiplier
        range = baseRange * (1.0 + CGFloat(level - 1) * BalanceConfig.ProtocolScaling.rangePerLevel)
        attackSpeed = baseAttackSpeed * (1.0 + CGFloat(level - 1) * BalanceConfig.ProtocolScaling.attackSpeedPerLevel)
    }

    /// Gold cost to upgrade to next level
    var upgradeCost: Int {
        guard level < BalanceConfig.maxUpgradeLevel else { return 0 }
        return BalanceConfig.ProtocolScaling.legacyUpgradeCostBase + (level * BalanceConfig.ProtocolScaling.legacyUpgradeCostPerLevel)
    }
