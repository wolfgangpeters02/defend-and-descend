...
final class MegaBoardSystem {
    ...
    func loadDefaultConfig() {
        ...
        print("[MegaBoardSystem] Loaded default config with \(config?.sectors.count ?? 0) sectors")
    }
    ...
    func loadConfig(from data: Data) throws {
        ...
        print("[MegaBoardSystem] Loaded config from JSON with \(config?.sectors.count ?? 0) sectors")
    }
    ...
    func unlockSector(_ sectorId: String, profile: inout PlayerProfile) -> Bool {
        ...
        guard result.canUnlock else {
            print("[MegaBoardSystem] Cannot unlock \(sectorId): \(result.reason ?? "unknown")")
            return false
        }
        ...
        print("[MegaBoardSystem] Unlocked sector: \(sector.displayName) for \(sector.unlockCost) Hash")
        return true
    }
