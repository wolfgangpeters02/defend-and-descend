...
struct SectorSchematicLibrary {
    ...
    /// RAM sector - Requires boss drop (Burst Protocol from Cyberboss)
    static let ram = SectorSchematic(
        sectorId: SectorID.ram.rawValue,
        displayName: "Memory Banks",
        description: "Multi-target processing protocols required",
        requiredProtocols: ["burst_protocol"]  // Dropped by Cyberboss (first boss)
    )
