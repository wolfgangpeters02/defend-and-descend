...
struct OverclockSystem {
    ...
    static let overclockDuration: TimeInterval = 60  // Seconds
    static let hashMultiplier: CGFloat = 2.0         // 2x hash generation
    static let threatMultiplier: CGFloat = 10.0      // 10x threat growth
    static let powerDemandMultiplier: CGFloat = 2.0  // 2x power demand
    ...
    static func activateOverclock(state: inout TDGameState) -> Bool {
        ...
        guard state.canOverclock else {
            print("[Overclock] Cannot activate - already active or boss present")
            return false
        }
        ...
        print("[Overclock] Activated! Duration: \(overclockDuration)s, Power required: \(state.overclockPowerRequired)")
        return true
    }
    ...
    private static func deactivateOverclock(state: inout TDGameState) {
        ...
        print("[Overclock] Deactivated - all towers re-enabled")
    }
    ...
    private static func updatePowerAllocation(state: inout TDGameState) {
        ...
        if !state.disabledTowerIds.isEmpty {
            print("[Overclock] Disabled \(state.disabledTowerIds.count) towers due to power shortage (deficit: \(-deficit)W)")
        }
    }
