...
// DEPRECATED: Use ComponentLevels instead
// This struct is maintained for backward compatibility during migration
// It now delegates to ComponentLevels for effect calculations

struct GlobalUpgrades: Codable, Equatable {
    var psuLevel: Int = 1           // Power capacity ceiling (→ Component.psu)
    var cpuLevel: Int = 1           // Affects Hash generation (→ Component.cpu)
    var ramLevel: Int = 1           // Affects efficiency regen (→ Component.ram)
    var coolingLevel: Int = 1       // Affects attack speed (→ Component.cache)
    var hddLevel: Int = 1           // Hash storage capacity (→ Component.storage)

    static var maxLevel: Int { BalanceConfig.Components.maxLevel }
    ...
    // MARK: - Migration to Component System

    /// Convert to new ComponentLevels
    func toComponentLevels() -> ComponentLevels {
        var levels = ComponentLevels()
        levels.psu = psuLevel
        levels.cpu = cpuLevel
        levels.ram = ramLevel
        levels.cache = coolingLevel  // Cooling → Cache (attack speed)
        levels.storage = hddLevel     // HDD → Storage
        // Other components start at level 1
        return levels
    }

    /// Create from ComponentLevels
    static func from(_ components: ComponentLevels) -> GlobalUpgrades {
        return GlobalUpgrades(
            psuLevel: components.psu,
            cpuLevel: components.cpu,
            ramLevel: components.ram,
            coolingLevel: components.cache,  // Cache → Cooling
            hddLevel: components.storage     // Storage → HDD
        )
    }
    ...
    static func powerCapacity(at level: Int) -> Int {
        return BalanceConfig.Components.psuCapacity(at: level)
    }
    ...
    static func psuUpgradeCost(at level: Int) -> Int? {
        return BalanceConfig.Components.upgradeCost(for: "psu", at: level)
    }
    ...
    static func hashStorageCapacity(at level: Int) -> Int {
        return BalanceConfig.Components.storageCapacity(at: level)
    }
    ...
    static func hddUpgradeCost(at level: Int) -> Int? {
        return BalanceConfig.Components.upgradeCost(for: "storage", at: level)
    }
    ...
    /// Uses centralized BalanceConfig values
    static func hashPerSecond(at level: Int) -> CGFloat {
        return BalanceConfig.HashEconomy.hashPerSecond(at: level)
    }
    ...
    static func cpuUpgradeCost(at level: Int) -> Int? {
        return BalanceConfig.Components.upgradeCost(for: "cpu", at: level)
    }
    ...
    /// Health at a given RAM level (base + perLevel × (level-1))
    static func healthBonus(at level: Int) -> CGFloat {
        return BalanceConfig.Components.ramBaseHealth + CGFloat(level - 1) * BalanceConfig.Components.ramHealthPerLevel
    }
    ...
    static func efficiencyRegenMultiplier(at level: Int) -> CGFloat {
        return BalanceConfig.Components.ramEfficiencyRegen(at: level)
    }
    ...
    static func ramUpgradeCost(at level: Int) -> Int? {
        return BalanceConfig.Components.upgradeCost(for: "ram", at: level)
    }
    ...
    static func fireRateMultiplier(at level: Int) -> CGFloat {
        return BalanceConfig.Components.cacheAttackSpeedMultiplier(at: level)
    }
    ...
    static func coolingUpgradeCost(at level: Int) -> Int? {
        return BalanceConfig.Components.upgradeCost(for: "cache", at: level)
    }
