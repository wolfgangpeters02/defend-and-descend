...
final class BlueprintDropSystem {
    ...
    func calculateDrop(
        ...
        guard let lootTable = LootTableLibrary.lootTable(for: bossId) else {
            print("[BlueprintDrop] No loot table for boss: \(bossId)")
            return .noDrop
        }
        ...
        guard !availableEntries.isEmpty else {
            print("[BlueprintDrop] Player owns all drops from \(bossId)")
            return DropResult(
                protocolId: nil,
        ...
        if isFirstKill && lootTable.guaranteeOnFirstKill {
            ...
            print("[BlueprintDrop] First kill of \(bossId) - guaranteed: \(guaranteedEntry?.protocolId ?? "none")")
            return DropResult(
        ...
        if killsSinceLastDrop >= LootTableLibrary.pityThreshold {
            ...
            print("[BlueprintDrop] Pity drop after \(killsSinceLastDrop) kills: \(pityEntry?.protocolId ?? "none")")
            return DropResult(
    ...
    private func rollForDrop(
        ...
        for drop in normalizedDrops {
            ...
            if roll < cumulative {
                print("[BlueprintDrop] Roll \(String(format: "%.3f", roll)) < \(String(format: "%.3f", cumulative)) - drop: \(drop.protocolId)")
                return DropResult(
        ...
        // No drop this time
        print("[BlueprintDrop] Roll \(String(format: "%.3f", roll)) - no drop (cumulative: \(String(format: "%.3f", cumulative)))")
        return DropResult(
            protocolId: nil,
            isFirstKill: isFirstKill,
            wasGuaranteed: false,
