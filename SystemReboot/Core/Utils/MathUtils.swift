...
enum NumberFormatUtils {
    /// Format number with compact notation (1K, 2.5M, etc.)
    /// Kicks in at 1000+ to prevent line breaks in UI
    ...
    /// Format hash display showing current/max (e.g., "12.5K / 25K")
    static func hashWithCap(current: Int, max: Int) -> String {
        return "\(compact(current)) / \(compact(max))"
    }
}
