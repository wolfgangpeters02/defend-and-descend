...
struct ZeroDaySystem {
    ...
    // MARK: - Constants (delegating to BalanceConfig)

    /// Minimum waves before Zero-Day can spawn (to let player set up)
    static var minWavesBeforeSpawn: Int { BalanceConfig.ZeroDay.minWavesBeforeSpawn }

    /// Efficiency drain rate per second while Zero-Day is active
    static var efficiencyDrainRate: CGFloat { BalanceConfig.ZeroDay.efficiencyDrainRate }

    /// Bonus Hash for defeating Zero-Day
    static var defeatHashBonus: Int { BalanceConfig.ZeroDay.defeatHashBonus }

    /// Efficiency restored when Zero-Day is defeated
    static var defeatEfficiencyRestore: Int { BalanceConfig.ZeroDay.defeatEfficiencyRestore }
    ...
    static func update(state: inout TDGameState, deltaTime: TimeInterval) -> Bool {
        ...
        if state.zeroDayActive && state.isZeroDayAlive {
            ...
            // Cap leak counter (100% efficiency loss)
            state.leakCounter = min(state.leakCounter, BalanceConfig.ZeroDay.maxLeakCounter)
    ...
    static func spawnZeroDay(state: inout TDGameState) {
        ...
        var zeroDayBoss = TDEnemy(
            ...
            pathIndex: 0,
            pathProgress: 0,
            health: BalanceConfig.ZeroDay.baseHealth,  // Very high health (immune to towers anyway)
            maxHealth: BalanceConfig.ZeroDay.baseHealth,
            speed: BalanceConfig.ZeroDay.speed,        // Very slow - gives player time to react
            damage: 0,              // Doesn't do direct damage
            goldValue: 0,           // No coins (reward is from Active mode)
            xpValue: 0,
            size: BalanceConfig.ZeroDay.size,          // Large, menacing
